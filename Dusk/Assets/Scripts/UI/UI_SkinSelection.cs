...
public class UI_SkinSelection : MonoBehaviour
{
    ...
    private UI_Difficulty difficultySelectionUI;
    ...
    private void Start()
    {
        ...
        difficultySelectionUI = mainMenuUI.GetComponentInChildren<UI_Difficulty>(true);
    }
    ...
    {
        bankText.text = "Bank: " + CoinsInBank();
    ...
    {
        if (HaveEnoughCoins(skinList[index].skinPrice) == false)
        {
    ...
    private int CoinsInBank() => PlayerPrefs.GetInt("TotalCoinsAmount");

    private bool HaveEnoughCoins(int price)
    {
        if (CoinsInBank() > price)
        {
            PlayerPrefs.SetInt("TotalCoinsAmount", CoinsInBank() - price);
            return true;
        }
